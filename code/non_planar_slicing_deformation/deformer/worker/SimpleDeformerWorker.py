...
class SimpleDeformerWorker(DeformerWorker):
    """
    ...
    def run(self) -> None:
        ...
        parameters = cast(KeyValueParameters, self.parameters)
        ...
        def rotation(radius: np.float64) -> np.float64:
            a = cast(np.float64, parameters["zeroth order", np.float64])  # TODO deal with None
            b = cast(np.float64, parameters["first order", np.float64])  # TODO deal with None
            c = cast(np.float64, parameters["second order", np.float64])  # TODO deal with None
